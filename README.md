...
- どのファイルにクラスを追加するか（`MatrixNode` / `OutputNode` / Port 実装）
- どのメソッド実装が必須か（`_from_spec`, `build`, `_from_output_spec`, `run` など）
- クラス名の手動登録（配列や辞書追記）が必要かどうか
  - `MatrixNode` / `OutputNode` は `__init_subclass__` による自動登録のため、通常は追記不要
